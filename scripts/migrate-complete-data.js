...
console.log('🔍 Starting comprehensive BLS data migration...');
...
async function migrateCompleteData() {
  try {
    ...
    try {
      ...
      const hasEmploymentColumn = columns.some(col => col.name === 'total_employment');
      ...
      for (const columnDef of additionalColumns) {
        ...
        if (!hasColumn) {
          ...
          db.exec(`ALTER TABLE occupation_data ADD COLUMN ${columnDef}`);
        }
      }
    } catch (error) {
                          ...
                          try {
                            await db.execute({
                              sql: `INSERT OR REPLACE INTO bls_special_tables (
                                occupation_code, table_number, table_name, data_year,                                projection_period, value, value_type, additional_data
                              ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                              args: [
                                detailCode,
                                tableNumber,
...
- Records with employment data: ${employmentDataCount.count}`);
    ...
    console.log('\n📋 Sample Enhanced Data:');    const sampleEmployment = db.prepare(`
...
try {
  const db = new Database(dbPath);
  const columns = db.prepare("PRAGMA table_info(bls_special_tables)").all();
  ...
  if (!hasAdditionalData) {
    ...
    db.exec('ALTER TABLE bls_special_tables ADD COLUMN additional_data TEXT');
  }
  db.close();
} catch (error) {
