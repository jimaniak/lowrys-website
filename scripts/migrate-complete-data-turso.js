...
console.log('🔍 Starting comprehensive BLS data migration (TURSO)...');
...
async function migrateCompleteData() {
  try {
    ...
    for (const [majorGroupCode, majorGroupData] of Object.entries(dateData)) {
      ...
      // Progress update
      if (occupationDataInserted % 5000 === 0) {
    ...
    const occupationCount = await db.execute({ sql: 'SELECT COUNT(*) as count FROM occupation_data WHERE data_year = ?', args: [parseInt(dataYear)] });
    const projectionCount = await db.execute('SELECT COUNT(*) as count FROM projections');
    const specialTableCount = await db.execute({ sql: 'SELECT COUNT(*) as count FROM bls_special_tables WHERE data_year = ?', args: [parseInt(dataYear)] });
    const employmentDataCount = await db.execute({ sql: 'SELECT COUNT(*) as count FROM occupation_data WHERE total_employment IS NOT NULL AND data_year = ?', args: [parseInt(dataYear)] });
    
    console.log(`Database totals:
- Occupation data: ${occupationCount.rows[0].count}
- Projections: ${projectionCount.rows[0].count}
- Special tables: ${specialTableCount.rows[0].count}
- Records with employment data: ${employmentDataCount.rows[0].count}`);
    
    // Sample the enhanced data
    console.log('\n📋 Sample Enhanced Data:');
    const sampleEmployment = await db.execute({
      sql: `SELECT occupation_code, total_employment, base_year_employment, projection_year_employment, 
    ...
    sampleEmployment.rows.forEach(row => {
...
async function prepareSchema() {
  try {
    const columns = await db.execute("PRAGMA table_info(bls_special_tables)");
    const hasAdditionalData = columns.rows.some(row => row[1] === 'additional_data');
    
    if (!hasAdditionalData) {
      console.log('🔧 Adding additional_data column to capture all BLS table data...');
      await db.execute('ALTER TABLE bls_special_tables ADD COLUMN additional_data TEXT');
    }
  } catch (error) {
