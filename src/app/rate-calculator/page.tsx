...
export default function EnhancedRateCalculator() {
  ...
  const [taxRate, setTaxRate] = useState<number>(25);  const [includeBenefits, setIncludeBenefits] = useState(false);
  const [benefits, setBenefits] = useState<number>(0);
  
  // Modal states
  const [showOccupationModal, setShowOccupationModal] = useState<boolean>(false);
  const [showJobOutlookModal, setShowJobOutlookModal] = useState<boolean>(false);
  const [showWageDataModal, setShowWageDataModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    const loadOccupations = async () => {
      try {
        setLoading(true);
        let url = '/api/rate-calculator?action=detailed-occupations';
  ...
  useEffect(() => {
    const loadRegions = async () => {
      ...
      try {
        const response = await fetch(`/api/rate-calculator?action=regions&occupation=${selectedDetailed}`);
        const data = await response.json();
  ...
  useEffect(() => {
    const loadWageData = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/rate-calculator?action=wage-data&occupation=${selectedDetailed}&region=${selectedRegion}`);
        if (!response.ok) {
        ...
        const projResponse = await fetch(`/api/rate-calculator?action=projections&occupation=${selectedDetailed}`);
        if (projResponse.ok) {
        ...
        // Load hierarchy data
        const hierarchyResponse = await fetch(`/api/rate-calculator?action=hierarchy&occupation=${selectedDetailed}`);
    ...
    loadWageData();
  }, [selectedDetailed, selectedRegion]);  // Calculations
  ...
  const blsDaily = wageData && wageData.wage.mean_annual && billableDays > 0 ? wageData.wage.mean_annual / billableDays : null;
  
  // Calculate actual net take-home (what you'll actually pocket)
  const actualNetTakeHome = income; // This is what the user will actually take home after all expenses and taxes
...
Source: Bureau of Labor Statistics Employment Projections`;
  ...
  const applyPreset = (presetType: 'conservative' | 'moderate' | 'aggressive') => {
    // BLS data is gross total compensation - estimate net take-home as ~60-70% after taxes, benefits, etc.
    const blsGrossCompensation = wageData?.wage.mean_annual || 100000;
    const estimatedEmployeeNetTakeHome = Math.round(blsGrossCompensation * 0.65); // Conservative net estimate
    
    switch (presetType) {
      case 'conservative':
        setIncome(Math.round(estimatedEmployeeNetTakeHome * 0.85)); // Target 85% of estimated employee net
        setExpenses(8000);
        setBillableDays(220); // More working days for stability
        setTaxRate(22);
        setIncludeBenefits(true);
        setBenefits(wageData?.benefits.avg_annual || 15000);
        break;
      case 'moderate':
        setIncome(estimatedEmployeeNetTakeHome); // Match estimated employee net take-home
        setExpenses(12000);
        setBillableDays(200); // Standard consulting schedule
        setTaxRate(25);
        setIncludeBenefits(true);
        setBenefits(wageData?.benefits.avg_annual || 18000);
        break;
      case 'aggressive':        setIncome(Math.round(estimatedEmployeeNetTakeHome * 1.15)); // Target 115% of estimated employee net
        setExpenses(15000);
        setBillableDays(160); // Fewer days, higher rates
        setTaxRate(28);
        setIncludeBenefits(true);
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-white flex items-center justify-center py-8 px-4">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-200 p-6 lg:p-10 relative overflow-hidden transition-all duration-500 hover:shadow-2xl">
        ...
        <div className="mb-6 flex justify-center relative z-20">
          <div className="inline-flex rounded-xl bg-slate-100 border border-slate-300 shadow-sm overflow-hidden">            <button
              className={`px-5 py-2 font-semibold transition-colors cursor-pointer ${
        ...
        </div>        {/* Step 0: Choose Your Occupation */}
        <div className="mb-8 relative z-10">
          <div className="bg-gradient-to-r from-slate-50 to-blue-50 border border-slate-300 rounded-xl p-6 mb-6">
            <h2 className="text-xl font-bold text-slate-700 mb-4 flex items-center gap-2">
              <span className="bg-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold">0</span>
              Choose Your Occupation & Location
            </h2>
            <div className="text-sm text-slate-600 mb-4">
              First, select your occupation and location to get relevant BLS benchmark data.
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-slate-700 font-semibold mb-2">Search & Select Occupation</label>
                <input
                  type="text"
        ...
        {wageData && (
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-300 rounded-xl p-4 mb-4 shadow-sm">
              ...
              </div>              <div className="text-xs text-indigo-600 mt-2">
          ...
          {wageData && (
            <div className="bg-gradient-to-r from-blue-50 to-slate-50 border border-blue-300 rounded-xl p-4 flex flex-col gap-1 shadow-inner mb-6">              
              ...
              {projectionData ? (
                ...
                </div>              )}              <div className="text-xs text-blue-600 mt-1">
        ...
        {/* Input Fields */}
        {employmentType === 'consulting' && (
          <div className="mb-8 relative z-10">            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-slate-700">Your Details</h3>
              {wageData && (
                <div className="flex flex-col items-end">
                  <div className="text-xs text-slate-600 mb-1 font-medium">Quick Setup Templates:</div>
                  <div className="flex gap-1">
                    <button
                      ...
                      title="Conservative Strategy: Lower risk, stable income (220+ billable days, modest rates)"
                    >
                      🛡️ Conservative
                    </button>
                    <button
                      ...
                      title="Balanced Strategy: Market rates with standard business model (200 billable days)"
                    >
                      ⚖️ Market Rate
                    </button>
                    <button
                      ...
                      title="Premium Strategy: High-value positioning (160 billable days, premium rates)"
                    >
                      🚀 Premium
                    </button>
                  </div>                  <div className="text-xs text-slate-500 mt-1 text-right">Click to auto-fill fields with proven strategies</div>
                </div>
              )}
            </div>
              {/* Template Explanation */}
            {wageData && (
              <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">                <div className="flex items-start gap-2">
                  <span className="text-blue-600 mt-0.5">💡</span>
                  <div className="text-sm text-blue-800">
                    <div className="mb-2">
                      <strong>Templates estimate your net take-home goals:</strong> Based on BLS gross compensation 
                      (${wageData.wage.mean_annual?.toLocaleString()}), templates estimate what you'd take home as an employee (~65%) 
                      and set your net target accordingly.
                    </div>
                    <strong>New to consulting?</strong> Use the templates above to get started with proven pricing strategies:
                    <div className="mt-1 text-xs space-y-1">
                      <div><strong>Conservative:</strong> 85% of estimated employee net, more working days (lower risk)</div>
                      <div><strong>Market Rate:</strong> Match estimated employee net take-home (industry standard)</div>
                      <div><strong>Premium:</strong> 115% of estimated employee net, fewer days (high-value positioning)</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="block text-slate-700 font-semibold mb-1">
                  <span className="text-blue-600 text-sm">1.</span> Annual Net Income Target ($)
                </label>
                <input
                  type="number"
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200"
                ...
                <div className="text-xs text-slate-600 mt-1">
                  What you want to actually take home after all business costs and taxes
                </div>
              </div>
              ...
              <div>
                <label className="block text-slate-700 font-semibold mb-1">
                  <span className="text-blue-600 text-sm">2.</span> Billable Days per Year
                </label>
                <input
                  type="number"
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200"
                ...
                <div className="text-xs text-blue-600 mt-1">
                  How many days will you actually bill clients? (Typical: 180-220)
                </div>
              </div>
              ...
              <div>
                <label className="block text-slate-700 font-semibold mb-1">
                  <span className="text-blue-600 text-sm">3.</span> Annual Business Expenses ($)
                </label>
                <input
                  type="number"
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200"
                ...
                />
                <div className="text-xs text-blue-600 mt-1">
                  Office, equipment, software, marketing, etc.
                </div>
              </div>
              ...
              <div>
                <label className="block text-slate-700 font-semibold mb-1">
                  <span className="text-blue-600 text-sm">4.</span> Tax Rate (%)
                </label>
                <input
                  type="number"
                  className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-200"
                ...
                />
                <div className="text-xs text-blue-600 mt-1">
                  Combined federal, state, local, self-employment taxes
                </div>
              </div>
              
              {/* Step 5: Benefits (spans full width) */}
              <div className="md:col-span-2">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  />                  <label htmlFor="includeBenefits" className="text-slate-700 font-semibold">
                    Add Self-Funded Benefits (health, retirement, etc.)
                  </label>
                </div>
                <div className="text-xs text-slate-500 mb-3 pl-6">
                  As a consultant, you'll need to pay for your own benefits that an employer would typically provide
                </div>
                
                {includeBenefits && (
                  <div className="pl-6">                    <label className="block text-slate-700 font-semibold mb-1">
                      Annual Self-Funded Benefits Cost ($)
                      {wageData && wageData.benefits.avg_annual && (
                        <span className="ml-2 text-xs text-blue-600">
                          (Typical: ${wageData.benefits.avg_annual.toLocaleString()})
                        </span>
                      )}
                    </label>
                    <input
                    ...
                    />
                    <div className="text-xs text-blue-600 mt-1">Health insurance, retirement contributions, etc.</div>
                  </div>
                )}              </div>
            </div>
          </div>
        ...
        <div className="mt-8 flex flex-col items-center">
          ...
          {employmentType === 'consulting' && dailyRate > 0 && (
            <div className="w-full max-w-md mt-4 bg-slate-50 border border-slate-200 rounded-xl p-6">
              <h3 className="text-lg font-bold text-slate-700 mb-4 text-center">Revenue Breakdown</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between font-bold text-blue-700 bg-blue-50 px-2 py-1 rounded">
                  ...
                  <span>${grossNeeded.toLocaleString()}</span>
                </div>
                {wageData && wageData.wage.mean_annual && (
                  <div className="flex justify-between text-xs text-slate-500 px-2">
                    <span>vs. BLS Average Gross Comp:</span>
                    <span>${wageData.wage.mean_annual.toLocaleString()} 
                      <span className={`ml-1 font-bold ${grossNeeded > wageData.wage.mean_annual ? 'text-green-600' : 'text-red-600'}`}>
                        ({grossNeeded > wageData.wage.mean_annual ? '+' : ''}{((grossNeeded - wageData.wage.mean_annual) / wageData.wage.mean_annual * 100).toFixed(1)}%)
                      </span>
                    </span>
                  </div>
                )}
                <hr className="border-slate-300 my-3" />                <div className="text-xs text-slate-600 font-semibold mb-2">How much gross revenue you need:</div>
                <div className="flex justify-between">
                  <span className="text-slate-600">Your Net Take-Home Target:</span>
                  <span className="font-semibold text-green-700">${income.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-xs text-slate-500">
                  <span className="pl-2">+ Business Expenses:</span>
                  <span>${expenses.toLocaleString()}</span>
                </div>
                {includeBenefits && (
                  <div className="flex justify-between text-xs text-slate-500">
                    <span className="pl-2">+ Self-Funded Benefits:</span>
                    <span>${benefits.toLocaleString()}</span>
                  </div>
                )}
                <div className="flex justify-between text-xs text-slate-500">
                  <span className="pl-2">+ Taxes ({taxRate}%):</span>
                  <span>${((grossNeeded - (income + totalExpenses))).toLocaleString()}</span>
                </div>
                <hr className="border-slate-300" />
                <div className="flex justify-between text-base font-bold">
                  <span className="text-slate-700">= Total Gross Revenue:</span>
                ...
                <div className="flex justify-between text-base font-bold">
                  ...
                  <span className="text-blue-600">${dailyRate.toLocaleString(undefined, { maximumFractionDigits: 2 })}/day</span>
                </div>
              </div>
            </div>
          ...
          {(dailyRate > 0 || blsDaily) && (
            <div className="w-full max-w-md mt-6 space-y-2 relative z-20">
              ...
              <div className="grid grid-cols-3 gap-3">                <button
                  onClick={() => copyToClipboard(generateShareableResults())}
                  className="group flex flex-col items-center justify-center bg-slate-50 border-2 border-slate-300 hover:border-sky-400 hover:bg-sky-50 text-slate-600 hover:text-sky-700 font-medium py-3 px-3 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md cursor-pointer"
                ...
                <button
                  ...
                  className="group flex flex-col items-center justify-center bg-slate-50 border-2 border-slate-300 hover:border-emerald-400 hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 font-medium py-3 px-3 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md"
                ...
                <button
                  ...
                  className="group flex flex-col items-center justify-center bg-slate-50 border-2 border-slate-300 hover:border-indigo-400 hover:bg-indigo-50 text-slate-600 hover:text-indigo-700 font-medium py-3 px-3 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md"
