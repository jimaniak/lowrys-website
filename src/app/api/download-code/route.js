// src/app/api/download-code/route.js
...
export async function POST(request) {
  try {
    ...
    const { passcode, category = 'code' } = body;
    if (!passcode) {
    ...
    // Serve the file
    const filePath = path.join(process.cwd(), 'public', 'documents', 'code-samples.zip');
    try {
      ...
      return NextResponse.json({ success: false, message: 'Code file not found.' }, { status: 500 });
    }
  } catch (error) {
