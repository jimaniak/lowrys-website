  ...
  try {
    switch (action) {      case 'major-groups':
      ...
      case 'occupations-by-major':
        ...
        // Get MINOR and BROAD categories for navigation (not final occupations)
        const categoriesForMajor = await db.execute({
        ...
        return NextResponse.json({ 
          occupations: categoriesForMajor.rows
        });
      ...
      case 'detailed-occupations':
        ...
        // If no DETAILED records found, check for direct OCCUPATION records
        if (occupationsResult.rows.length === 0 && minorCode && minorCode !== 'ALL') {
          const directOccupationsQuery = `
            SELECT code, name, occupation_type, category 
            FROM occupations 
            WHERE category = 'OCCUPATION'
              ...
              AND TRIM(name) != ''
            ORDER BY name`;
          const directOccupationsResult = await db.execute({ 
            sql: directOccupationsQuery, 
            args: [minorCode, minorCode.substring(0, 4) + '%'] 
          });
          
          // Mark these as final level since they go directly from MINOR to OCCUPATION
          const finalOccupations = directOccupationsResult.rows.map((row: any) => ({
            ...row,
            is_final_level: true
          }));
          ...
          return NextResponse.json({ 
            ...
            hierarchy_depth: 'direct_to_occupation'
          });
        }
        
        return NextResponse.json({ 
          occupations: occupationsResult.rows,
          hierarchy_depth: 'has_detailed_level'
        });
      ...
      case 'occupations-by-minor':
        const minorCodeParam = searchParams.get('minorCode');
        if (!minorCodeParam) {
          return NextResponse.json({ error: 'Minor code required' }, { status: 400 });
        }
        
        // Get all OCCUPATION jobs under this MINOR/BROAD parent (through DETAILED parents)
        const occupationsByMinor = await db.execute({
          sql: `SELECT code, name, occupation_type, category 
                FROM occupations 
                WHERE category = 'OCCUPATION'
                  AND code LIKE ?
                  ...
                  AND TRIM(name) != ''
                ORDER BY name`,
          args: [minorCodeParam.substring(0, 5) + '%']
        });
        
        return NextResponse.json({ 
          occupations: occupationsByMinor.rows
        });

      case 'all-occupations-by-major':
        const majorCodeForOccupations = searchParams.get('majorCode');
        if (!majorCodeForOccupations) {
          return NextResponse.json({ error: 'Major code required' }, { status: 400 });
        }
        
        // Get all OCCUPATION jobs under this major group
        const allOccupationsForMajor = await db.execute({
          sql: `SELECT code, name, occupation_type, category 
                FROM occupations 
                WHERE category = 'OCCUPATION'
                  AND code LIKE ?
                  ...
                  AND TRIM(name) != ''
                ORDER BY name`,
          args: [majorCodeForOccupations.substring(0, 2) + '%']
        });
        
        return NextResponse.json({ 
          occupations: allOccupationsForMajor.rows
        });

      case 'all-occupations':
        // Get ALL occupation jobs (no filter)
        const allOccupations = await db.execute({
          sql: `SELECT code, name, occupation_type, category 
                FROM occupations 
                WHERE category = 'OCCUPATION'
                  AND name IS NOT NULL 
        ...
        return NextResponse.json({ 
          occupations: allOccupations.rows
        });
      ...
      case 'check-table':
        ...
        const tableLimit = searchParams.get('limit') || '10';
        ...
        try {
          ...
          const sampleResult = await db.execute(`SELECT * FROM ${tableName} LIMIT ${tableLimit}`);
          ...
          return NextResponse.json({ 
            error: `Error checking table ${tableName}: ${error}` 
          }, { status: 500 });
        }
      ...
      case 'hierarchy-occupations':
        ...
        return NextResponse.json({ occupations: detailedOccupationResult.rows });
      ...
      case 'search-occupations':
        ...
        try {
          // Search through OCCUPATION category records by name
          const searchResult = await db.execute({
            sql: `SELECT code, name, category, occupation_type 
                  FROM occupations 
                  WHERE category = 'OCCUPATION' 
                    AND name LIKE ? 
                  ORDER BY name 
      ...
      // REMOVED OLD ENDPOINTS - IF YOU SEE 404 ERRORS, FIND WHERE THESE ARE BEING CALLED:
      // case 'get-major-categories':
      ...
      // REMOVED: case 'get-minor-categories':
